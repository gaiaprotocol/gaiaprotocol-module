...
export default class TradeMaterialTabContent extends DomNode {
    ...
    constructor(address, tradeType) {
        ...
        this.append(el("section.amount", el("header", el("h3", "Amount"), el(".current-balance", el("span.label", "Your Balance"), this.balanceDisplay = el("span.balance", new AppCompConfig.LoadingSpinner()))), this.amountInput = new Input({
            placeholder: "0.0000",
            value: "1",
            onChange: () => this.loadAllPrices(),
        })), el("section.price", el("header", el("h3", `${StringUtils.capitalize(tradeType)} Price`), el(".unit-price", el("span.label", "Unit Price"), this.unitPriceDisplay = el("span.price", new AppCompConfig.LoadingSpinner()))), el("ul", el("li", el("span.label", "Total Price"), this.totalPriceDisplay = el("span.price")), el("li", el("span.label", "Fee"), this.feeDisplay = el("span.price")))), el("section.fee-recipients", el("header", el("h3", "Fee Recipients"), el("span.recipient-count", "2 recipients")), this.feeRecipientList = new FeeRecipientList()));
        this.loadAllPrices();
    }
    loadAllPrices() {
        this.loadBalance();
